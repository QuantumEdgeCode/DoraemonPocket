...
项目名称:python 
集成开发环境:PyCharm
文件名:获取日月年k线优化版240118-y2.py
生成时间:2024-01-18 下午 01:03:08
创建用户:musk	
AIEPN Inc
'''
...
''')
# 获取用户输入的kit值
kit = input("请输入kit值（例如：101、102、103、104、1、5）: ")

# 根据kit值设置不同的数据目录
if kit == "101":
    data_directory = f"./data/a/day/{current_date}"
elif kit == "102":
    data_directory = f"./data/a/zhou/{current_date}"
elif kit == "103":
    data_directory = f"./data/a/yue/{current_date}"
elif kit == "104":
    data_directory = f"./data/a/3mo/{current_date}"
elif kit == "1":
    data_directory = f"./data/a/1分钟级别/{current_date}"
elif kit == "5":
    data_directory = f"./data/a/5分钟级别/{current_date}"
else:
    raise ValueError("不支持的kit值，请输入101、102、103、104、1或5")
...
# # 打包目录为tar.xz文件
# tar_filename = f"{data_directory}.tar.xz"
# shutil.make_archive(data_directory, 'xztar', data_directory)
#
# # 打印日志和消息
# logging.info(f"目录 {data_directory} 已打包为 {tar_filename}")
# print(f"目录 {data_directory} 已打包为 {tar_filename}")
