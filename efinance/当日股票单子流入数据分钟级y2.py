...
'''
项目名称:python 
文件名:当日股票单子流入数据分钟级y2.py
生成时间:2023/12/7 19:43:06
创建用户:musk	
AIEPN Inc
'''
import os
...
# 读取股票代码列表
with open('szcode.txt', 'r') as file:
...
for code in stock_codes:
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            # 打印成功信息
            print(f'成功获取股票 {code} 的数据。保存在: {save_path}')
            ...
            # 打印错误信息
            print(f'获取股票 {code} 数据时发生错误。重试中... (重试 {retries + 1}/{MAX_RETRIES})')

            # 记录错误日志
            logging.error(f'获取股票 {code} 数据时发生错误: {str(e)}')
        ...
        # 打印错误信息，超过最大重试次数
        print(f'在 {MAX_RETRIES} 次重试后无法获取股票 {code} 数据。继续下一个股票。')
